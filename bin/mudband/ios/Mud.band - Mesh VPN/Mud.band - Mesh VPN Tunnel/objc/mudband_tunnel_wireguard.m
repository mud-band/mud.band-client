    ...
    struct wireguard_peer *peer, bool need_multipath)
{
	...
	if (errno == ENOBUFS) {
        ...
        vtc_log(wg_vl, 0, "BANDEC_00325: sendto(2) to %s:%d failed: %d %s",
...
wireguard_peer_snapshot_run(struct wireguard_device *device)
{
    ...
    if (device->peers_count >= 65536) {
        vtc_log(wg_vl, 0, "BANDEC_00881: Too many peers.");
        return;
    }
    ...
    if (new_peer_snapshots == NULL) {
        vtc_log(wg_vl, 0,
            "BANDEC_00882: Failed to allocate memory for"
            " peer snapshots.");
        return;
    }
    ...
    struct wireguard_sockaddr *wsin)
{
	...
	vtc_log(wg_vl, 0,
		"BANDEC_00895: sendto(2) to %s:%d failed: %d %s",
		inet_ntoa(sin.sin_addr), pr->endpoints[x].port,
		errno, strerror(errno));
	return (-1);
    }
