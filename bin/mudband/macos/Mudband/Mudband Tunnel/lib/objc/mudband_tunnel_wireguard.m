    ...
    struct pbuf *pbuf, struct wireguard_peer *pr)
{
    ...
    for (x = 0 ; x < pr->n_endpoints; x++) {
		...
		return (-1);
            ...
            vtc_log(wg_vl, 0,
                    "BANDEC_00369: sendto(2) to %s:%d failed: %d %s",
    ...
    struct wireguard_iface_peer *p, int *peer_index)
{
    ...
    *peer_index = WIREGUARD_IFACE_INVALID_INDEX;
    ...
    r = wireguard_base64_decode(p->public_key, public_key, &public_key_len);
    if (!r || public_key_len != WIREGUARD_PUBLIC_KEY_LEN) {
        vtc_log(wg_vl, 0, "BANDEC_00371: Invalid public key %s",
                p->public_key);
        return (-1);
    }
    ...
    if (peer == NULL) {
        vtc_log(wg_vl, 0, "BANDEC_00372: No room for new peer");
        return (-1);
    }
    r = wireguard_peer_init(device, peer, public_key, p->preshared_key);
    if (!r) {
        vtc_log(wg_vl, 0,
                "BANDEC_00373: wireguard_peer_init() failed");
        return (-1);
    }
                             ...
                             struct mudband_tunnel_bandconf *cnf)
{
    ...
    for (i = 0; i < n_peers; i++) {
        ...
        if (old_peer == NULL) {
            ...
            if (r != 0) {
                vtc_log(wg_vl, 0,
                        "BANDEC_00376: wireguard_iface_add_peer()"
                        " failed: r %d", r);
                n_failure++;
                continue;
            }
            ...
            if (r != 0) {
                vtc_log(wg_vl, 0,
                        "BANDEC_00377: wireguard_iface_connect()"
                        " failed: r %d", r);
                n_failure++;
                continue;
            }
...
wireguard_peer_snapshot_run(struct wireguard_device *device)
{
    ...
    if (device->peers_count >= 65536) {
        vtc_log(wg_vl, 0, "BANDEC_00886: Too many peers.");
        return;
    }
    ...
    if (new_peer_snapshots == NULL) {
        vtc_log(wg_vl, 0,
            "BANDEC_00887: Failed to allocate memory for"
            " peer snapshots.");
        return;
    }
    ...
    struct wireguard_sockaddr *wsin)
{
	...
	vtc_log(wg_vl, 0,
		"BANDEC_00897: sendto(2) to %s:%d failed: %d %s",
		inet_ntoa(sin.sin_addr), pr->endpoints[x].port,
		errno, strerror(errno));
	return (-1);
    }
    ...
    size_t data_len, struct wireguard_sockaddr *wsin)
{
    ...
    if (keypair == NULL) {
        ...
        vtc_log(wg_vl, 1, "BANDEC_00378: No keypair found.");
        return;
    }
    if ((keypair->receiving_valid) &&
        ...
        if (pbuf == NULL) {
            vtc_log(wg_vl, 0, "BANDEC_00379: OOM");
            return;
        }
        // Decrypt the packet
...
void
mudband_tunnel_iface_need_sync(void)
{
